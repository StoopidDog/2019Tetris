...
public class CardMove2P : MonoBehaviour {
    ...
    void FixedUpdate()
    {
        if (Input.GetKeyDown(KeyCode.LeftArrow) && doAction)
        {
            moveDone = false;
            doAction = false;
            StartCoroutine(CardMovingAnim(CardPosition));
        }
        if (Input.GetKeyDown(KeyCode.RightArrow) && doAction)
        {
            moveDone = false;
            doAction = false;
        ...
        if (C.sortingOrder == 0)
        {
            GameManager.instance.Ch2P = CharacterNum;
        }
    }
